...
- persist bookings in local storage and reopen them later without losing the intended wall-clock meaning, that is, the date and time as the user expects to read them in the selected timezone.
...
1. The user thinks in wall-clock time for the selected timezone.
...
The preview is therefore not just decorative. It shows the actual occurrence expansion that should result from the submitted data.
...
### The Preview Is A Real Expansion Of The Recurrence

The preview is not just a visual summary of the form fields. It performs a real expansion of the booking data into concrete occurrence instants, based on the recurrence rule, the selected timezone, the configured start times, and the duration.

In that sense, the preview acts as a frontend representation of the same business semantics that the backend is expected to apply when it receives the booking payload. It does not simply display the raw input values. It computes the effective instants generated by the rule and then renders those instants on the timeline.

The actual recurrence expansion logic lives in [src/app/utils/timeline-preview.utils.ts](src/app/utils/timeline-preview.utils.ts), while [src/app/components/timeline-preview.component.ts](src/app/components/timeline-preview.component.ts) is responsible for rendering the computed result.
